...
# for (test_index in repeat_folds) {
#     i = i + 1
dataTrain <- train_data[, -c(2215, 2217, 2218)]
dataTest <- test_data[, -c(2215, 2217, 2218)]
OVO_SVM <- train_test_SVM_OVO(dataTrain, dataTest, inner_fold)
# OVO_SVM$Fold = i
saveRDS(
    OVO_SVM, 
    paste0(output_path,'/','results.rds')
)
write.csv(
    OVO_SVM, 
    paste0(output_path,'/','results.csv')
)
