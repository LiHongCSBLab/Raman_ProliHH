...
train_data = train_data[,-c(2215, 2217, 2218)]
tmp_sd = apply( train_data[, -ncol(train_data)], 2, sd )
ignore = (1:length(tmp_sd))[tmp_sd <  1.5e-04]
train_data = train_data[, -ignore]
...
svmPHHP1 = train(Group~., data=phhp1Train,method="lda2", trControl=fitControl )
svmPHHP4 = train(Group~., data=phhp4Train,method="lda2", trControl=fitControl )
svmPHHP9 = train(Group~., data=phhp9Train,method="lda2", trControl=fitControl )
svmP1P4 = train(Group~., data=p1p4Train,method="lda2", trControl=fitControl )
svmP1P9 = train(Group~., data=p1p9Train,method="lda2", trControl=fitControl )
svmP4P9 = train(Group~., data=p4p9Train,method="lda2", trControl=fitControl )

stopCluster(cl)
toc()
print("Training finished.")

svmOVOpred = data.frame(fulldataTest[,ncol(fulldataTest)])
svmOVOpred$PHHP1 = predict(svmPHHP1, fulldataTest[,-ncol(fulldataTest)], verbose = TRUE)
svmOVOpred$PHHP4 = predict(svmPHHP4, fulldataTest[,-ncol(fulldataTest)], verbose = TRUE)
svmOVOpred$PHHP9 = predict(svmPHHP9, fulldataTest[,-ncol(fulldataTest)], verbose = TRUE)
svmOVOpred$P1P4 = predict(svmP1P4, fulldataTest[,-ncol(fulldataTest)], verbose = TRUE)
svmOVOpred$P1P9 = predict(svmP1P9, fulldataTest[,-ncol(fulldataTest)], verbose = TRUE)
svmOVOpred$P4P9 = predict(svmP4P9, fulldataTest[,-ncol(fulldataTest)], verbose = TRUE)
...
for (trainIndex in repeat_folds) {
    ...
    print("Fold:")
    print(i)
    dataTrain <- train_data[ trainIndex, ]
    dataTest <- train_data[ -trainIndex, ]
